...
def apply_rotary_emb(x: torch.Tensor, freqs_cis: torch.Tensor) -> torch.Tensor:
    ...
    x_ = torch.view_as_complex(
        torch.stack(torch.chunk(x.transpose(1, 2).float(), 2, dim=-1),
                    dim=-1))
    freqs_cis = freqs_cis.to(x_.dtype)
    tmp = x_ * freqs_cis
    x_out = torch.view_as_real(tmp)
...
class GemmaForCausalLM(nn.Module):
    ...
    def my_index_select(self, input, dim, index):
        ...
        assert isinstance(index, torch.Tensor) and index.dtype == torch.int64, "Index must be a LongTensor"
        
        dim = dim.to('mlu')
        # Get the size of the input tensor
        sizes = list(input.size())

        # Create an empty list to store selected elements
        selected = []

        # Iterate over the specified dimension
        for i in range(sizes[dim]):
            if i in index:
                selected.append(input.index_select(dim, torch.tensor(i)))

        # Concatenate the selected elements along the specified dimension
        result = torch.cat(selected, dim=dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.embedder(input_token_ids) # <- torch.bfloat16
        tmp = self.config.hidden_size**0.5
